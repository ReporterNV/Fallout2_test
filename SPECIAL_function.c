...
for (struct Person_list *i=list;i!=NULL;i=i->next){
	...
	((i->NPC.Age<=30000)&&(i->NPC.Age>=0))? (fprintf(file,"\t\t\t Age: %i", i->NPC.Age)):(fprintf(file,"\t\t\t Age: ?"));
	((i->NPC.Gender=='M')||(i->NPC.Gender=='m'))? (fprintf(file,"\t\t\t Gender: Male")):(((i->NPC.Gender=='F')||(i->NPC.Gender=='f'))? (fprintf(file,"\t\t\t Gender: Female")) : (fprintf(file,"\t\t\t Gender=?")));

	fprintf(file,"\n::: Statistics :::");
	fprintf(file,"\n     Strength: %d",i->NPC.info.S); fprintf(file,"       Hit Points: %d",15+(i->NPC.info.E*2)+i->NPC.info.S); fprintf(file,"              Sequence: %d",i->NPC.info.P*2);
	fprintf(file,"\n   Perception: %d",i->NPC.info.P); fprintf(file,"      Armor Class: %d",i->NPC.info.A);               fprintf(file,"           Healing Rate: %d",i->NPC.info.E/3);
	fprintf(file,"\n    Endurance: %d",i->NPC.info.E); fprintf(file,"    Action Points: %d",i->NPC.info.A/2+5);           fprintf(file,"        Critical Chance: %d",i->NPC.info.L);
	fprintf(file,"\n     Charisma: %d",i->NPC.info.C); fprintf(file,"     Melee Damage: %d",i->NPC.info.S/3+1);           fprintf(file,"           Carry Weight: %d",25+(i->NPC.info.S*25));
	fprintf(file,"\n Intelligence: %d",i->NPC.info.I); fprintf(file,"      Damage Res.: %d",0);
	fprintf(file,"\n      Agility: %d",i->NPC.info.A); fprintf(file,"   Radiation Res.: %d",i->NPC.info.A);
	fprintf(file,"\n         Luck: %d",i->NPC.info.L); fprintf(file,"      Poison Res.: %d",i->NPC.info.L);
